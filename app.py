...
st.write("This pie chart shows the churn distribution based on various features. Notably, we have a 79% churn rate compared to the bar graph of plans v/s churn, this is because here the data is taken by combining all the features when compared to the Pricing & plans v/s churn bar graph.")
...
# 4. Bar Plot for Customer Engagement vs Churn (with Fixed Green and Red Colors)

customer_engagement_churn = df.groupby('Churn')['CustomerEngagement'].mean().reset_index()
 
# Define a color map for the churn status: 0 = green, 1 = red

color_map = {0: '#32CD32', 1: '#FF4500'}  # Green for not churned, red for churned
 
# Create the bar plot using the fixed color map
...
# Apply the fixed color map to the bars

fig_bar.update_traces(marker_color=customer_engagement_churn['Churn'].map(color_map))
...
with st.sidebar.form("churn_prediction_form"):
    ...
    if submit_button:
        ...
        features = np.array([
            ...
            service_quality, competitor_offers, customer_engagement, billing_issues, service_changes, 

            usage_patterns, economic_factors, technology_trends, customer_sentiment

        ] + encode_features(pricing_and_plans, contractual_obligations, customer_lifecycle_stage)).reshape(1, -1)
        ...
        churn_probability = model.predict(scaled_features)[0][0]
